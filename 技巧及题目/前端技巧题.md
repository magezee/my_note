...
class Prom {
    ...
    constructor(fn) {
        ...
        this.resolveFns = []    // resolve函数队列
        this.rejectFns = []     // reject函数队列
        ...
        fn(resolve, reject)     // 将resolve和reject方法传给fn
    }

    then(fn) {
        // 需要返回一个Promise，如果状态为resolve则直接执行
        if(this.status === 'RESOLVED') {
            const result = fn(this.value)
            return Prom.resolve(result)
        }

        // 如果状态为PENGING，则推进队列中，resolve后统一执行
