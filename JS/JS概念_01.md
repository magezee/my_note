...
var声明的变量提升是提至块级作用域外的，而let不可以，因此可以在不同的作用域中使用同名变量不会互相影响
...
console.log(Person)			            // [Function: Person] { sex: 'boy' }
...
当构造函数return的是一个值类型，则无视return内容
...
var d = new demo()
console.log(d)  // demo { x: 1 }
```

而return的是一个引用类型时，则直接返回return的内容
